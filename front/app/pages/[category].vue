...
<template>
    <div class="min-h-screen bg-gray-50 font-sans">
            ...
            <section v-if="!loading && article?.article?.length > 0">
                    ...
                    >
                        <div class="rounded-2xl overflow-hidden bg-white border border-gray-100 shadow-sm hover:shadow-md transition-shadow h-full flex flex-col">
                            <div class="relative overflow-hidden h-44">
                                <img :src="item.image || '/placeholder.jpg'" class="w-full h-full object-cover group-hover:scale-105 transition-duration-500" />
                            </div>
                            <div class="p-4 flex flex-col gap-2 flex-1">
                                <h3 class="text-sm font-semibold text-gray-800 line-clamp-3 group-hover:text-red-600">{{ item.title }}</h3>
                ...
                <div class="grid grid-cols-12 gap-8">
                    <div class="col-span-12 lg:col-span-8 flex flex-col gap-5">
                        ...
                        >
                            <img :src="item.image || '/placeholder.jpg'" class="w-36 h-24 object-cover rounded-xl shrink-0" />
                            <div class="flex flex-col gap-1">
                                <span class="text-xs text-red-600 font-semibold uppercase">{{ item.category?.[0]?.name || "Berita" }}</span>
                                <h3 class="text-base font-semibold text-gray-800 line-clamp-2 group-hover:text-red-600">{{ item.title }}</h3>
                    ...
                    <aside class="col-span-12 lg:col-span-4">
                        <div class="sticky top-20">
                            <h2 class="text-sm font-bold text-gray-800 uppercase tracking-widest mb-5">Artikel Populer</h2>
                            <div class="flex flex-col gap-4">
                                <NuxtLink v-for="(item, index) in article.article.slice(0, 5)" :key="index" :to="`/article/${item.id}`" class="flex gap-3 items-start group">
                                    <span class="text-3xl font-extrabold text-gray-100 shrink-0 w-8">{{ index + 1 }}</span>
                                    <h4 class="text-sm font-semibold text-gray-800 line-clamp-3 group-hover:text-red-600">{{ item.title }}</h4>
                                </NuxtLink>
                            </div>
                        </div>
                    </aside>
        ...
        <div class="mt-10 mb-2 w-full flex items-center justify-center gap-3">
            <span v-if="article.meta.currentPage == 1" class="block hover:bg-gray-200 hover:text-700 duration-100 rounded-xl w-20 py-2 text-center font-semibold">Prev</span>
            <NuxtLink v-else :to="`/${route.params.category}/${Number(route.params.id) - 1}`" class="block hover:bg-gray-200 hover:text-700 duration-100 rounded-xl w-20 py-2 text-center font-semibold">
                Prev
            </NuxtLink>
            <div class="flex gap-2">
            </div>
            <span v-if="article.meta.currentPage >= article.meta.lastPage" class="block hover:bg-gray-200 hover:text-700 duration-100 rounded-xl w-20 py-2 text-center font-semibold">Next</span>
            <NuxtLink :to="`/${route.params.category}/${Number(route.params.id) + 1}`" v-else>
                Next
            </NuxtLink>
        </div>
    </div>
