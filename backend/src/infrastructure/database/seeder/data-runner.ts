...
const runDynamicSeeder = async () => {
    ...
    const fileLocation = process.argv[2].toLowerCase() + ".seeder.ts";
    if (!fileLocation) {
        console.error(
            "❌ Please provide a seeder file name. Example: bun db:seed:data article",
        );
        process.exit(1);
    }
    
    try {
        const filePath = path.join(process.cwd(), "src/infrastructure/database/seeder/data", fileLocation);
